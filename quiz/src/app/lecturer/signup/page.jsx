...
export default function LecturerSignup() {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      const response = await axios.post("/api/lecturer/signup", formData);
      if (response.data.success) {
        setSuccess("Account created! Check your email for a verification link.");
        setTimeout(() => router.push("/lecturer/login"), 3000);
      }
    } catch (err) {
    ...
    <main className="grid min-h-screen place-items-center p-4 gradient-mesh">
      <Card className="w-full max-w-md p-8">
        <div className="text-center">
          ...
          <h1 className="mt-2 text-3xl font-bold">Lecturer Sign up</h1>
          <p className="mt-2 text-sm text-muted-foreground">Create an account to start making quizzes.</p>
        </div>
        ...
        <form className="mt-6 grid gap-4" onSubmit={handleSignup}>
          <Field label="Full Name / Title"><Input name="username" type="text" required value={formData.username} onChange={handleChange} placeholder="Prof. Smith" /></Field>
        ...
        <p className="mt-6 text-center text-sm text-muted-foreground">
          ...
          <Link href="/lecturer/login" className="font-semibold text-primary hover:underline">Login</Link>
        </p>
      </Card>
    </main>
  );
